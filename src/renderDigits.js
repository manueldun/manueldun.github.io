...
function initDigits(gl) {
    const vertexShaderSource = `attribute vec3 a_position;
    ...
    let rotationMatrixUniform = gl.getUniformLocation(
        program,
        "u_rotationMatrix"
    );
    let objecPositionUniform = gl.getUniformLocation(
        program,
        "u_objectPosition"
    );
    ...
    async function loadData() {
        ...
        for (let i = 0; i < zeroBufferUris.length; i++) {
            zeroBufferPromises.push(
                getBinaryFile("/assets/", zeroBufferUris[i])
            );
        }
        ...
        let offsetzero =
            zeroObject.bufferViews[zeroObject.meshes[0].primitives[0].indices]
                .byteOffset;
        let sizezero =
            zeroObject.bufferViews[zeroObject.meshes[0].primitives[0].indices]
                .byteLength;

        const indexBufferzeroSlice = zeroBuffers[0].slice(
            offsetzero,
            offsetzero + sizezero
        );
        ...
        let offsetone =
            oneObject.bufferViews[oneObject.meshes[0].primitives[0].indices]
                .byteOffset;
        let sizeone =
            oneObject.bufferViews[oneObject.meshes[0].primitives[0].indices]
                .byteLength;
        const indexBufferoneSlice = unoBuffers[0].slice(
            offsetone,
            offsetone + sizeone
        );
        gl.bufferData(
        ...
        return (width, height) => {
            ...
            for (const digit of digits) {
                gl.uniform3fv(
                    objecPositionUniform,
                    digit.getPosition(Date.now())
                );

                gl.uniformMatrix4fv(
                    rotationMatrixUniform,
                    false,
                    digit.getRotation(Date.now())
                );
                if (digit.digit === 0) {
                    gl.bindBuffer(gl.ARRAY_BUFFER, zeroBuffer);
                    ...
                    const positionOffsetzero =
                        zeroObject.bufferViews[
                            zeroObject.meshes[0].primitives[0].attributes
                                .POSITION
                        ].byteOffset;
                    ...
                    const normalOffsetzero =
                        zeroObject.bufferViews[
                            zeroObject.meshes[0].primitives[0].attributes.NORMAL
                        ].byteOffset;
                    ...
                    const positionOffsetone =
                        oneObject.bufferViews[
                            oneObject.meshes[0].primitives[0].attributes
                                .POSITION
                        ].byteOffset;
                    ...
                    const normalOffsetone =
                        oneObject.bufferViews[
                            oneObject.meshes[0].primitives[0].attributes.NORMAL
                        ].byteOffset;
